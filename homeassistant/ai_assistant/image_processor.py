    ...
    for item in ui_answer["items"]:
        ...
        if item_type == "image":
            ...
            if prompt:
                ...
                logger.info(f"image_proc_002: Found image prompt (16:9, 2K): \033[36m{prompt[:50]}...\033[0m")
            ...
            for card in cards:
                if card.get("type") == "card" and card.get("image_prompt"):
                    ...
                    logger.info(f"image_proc_003: Found card prompt ({aspect_ratio}, 1K): \033[36m{prompt[:50]}...\033[0m")
    
    logger.info(f"image_proc_004: Collected \033[33m{len(all_tasks)}\033[0m image prompts")
    
    if all_tasks:
        logger.info(f"image_proc_005: Starting parallel generation for \033[33m{len(all_tasks)}\033[0m images")

...
for i, ip in enumerate(lamps, 1):
    logger.info(f"test_lamps_001: Testing lamp \033[33m{i}/3\033[0m at \033[36m{ip}\033[0m")
    try:
        bulb = Bulb(ip)
        ...
        available_methods = [m for m in methods if m in ['turn_on', 'turn_off', 'toggle', 'set_brightness', 'set_color_temp', 'set_rgb', 'set_hsv']]
        logger.info(f"test_lamps_004: Available methods: \033[33m{len(available_methods)}\033[0m")
        for method in sorted(available_methods):
            logger.info(f"test_lamps_005:   \033[35m{method}\033[0m")
                
    except Exception as e:
        logger.error(f"test_lamps_error_001: \033[31mConnection failed to \033[36m{ip}\033[31m: {str(e)}\033[0m")

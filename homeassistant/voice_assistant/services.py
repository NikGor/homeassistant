...
try:
    ...
    logger.warning(f"voice_assistant_import_warning: Voice dependencies not available: {e}")
...
class WakeWordService:
    ...
    def __init__(self, access_key: str, keyword_path: Optional[str] = None):
        ...
        if not VOICE_DEPENDENCIES_AVAILABLE:
            raise ImportError("Voice dependencies not available - run locally with 'make install-voice'")
        ...
        try:
            ...
            logger.error(f"voice_assistant_error_001: Failed to initialize wake word detector: {e}")
            raise
    ...
    def start_listening(self, callback=None):
        ...
        try:
            ...
            while self.is_listening:
                ...
                if keyword_index >= 0:
                    logger.info(f"voice_assistant_003: Wake word detected (index: {keyword_index})")
                    if callback:
...
class VoiceProcessingService:
    ...
    def record_audio(self, duration: int = None, device_id: Optional[int] = None) -> str:
    ...
    def __init__(self):
        ...
        if access_key and VOICE_DEPENDENCIES_AVAILABLE:
            try:
                ...
                logger.error(f"voice_assistant_error_009: Wake word service unavailable: {e}")
        elif not VOICE_DEPENDENCIES_AVAILABLE:
            logger.info("voice_assistant_info_001: Voice dependencies not available - voice assistant disabled in Docker")
    ...
    def run_continuous_listening(self):
        ...
        def wake_word_callback(keyword_index):
            ...
            try:
                ...
                logger.error(f"voice_assistant_error_009: Voice interaction failed: {e}")
